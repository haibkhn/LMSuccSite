...
def prepare_structure_data(df):
    ...
    # Use large phi/psi angles as they cover more context
    phi = df['phi_large'].apply(eval).values  # Convert string representation to list
    psi = df['psi_large'].apply(eval).values
    
    # Convert secondary structure one-hot encoding
    ss = np.column_stack((df['E'], df['H'], df['L']))
    
    # Stack all features
    phi = np.array([np.pad(x, (0, 33 - len(x)), 'constant') for x in phi])
    psi = np.array([np.pad(x, (0, 33 - len(x)), 'constant') for x in psi])
    
    return sasa, phi, psi, ss

class AngleProcessingLayer(tf.keras.layers.Layer):
    """Custom layer to process angle features"""
...
def create_structure_model():
    ...
    # Phi angles input
    phi_input = tf.keras.layers.Input(shape=(33,))
    phi_features = AngleProcessingLayer(16)(phi_input)
    phi_features = tf.keras.layers.Dense(16, activation='relu')(phi_features)
    
    # Psi angles input
    psi_input = tf.keras.layers.Input(shape=(33,))
    psi_features = AngleProcessingLayer(16)(psi_input)
    psi_features = tf.keras.layers.Dense(16, activation='relu')(psi_features)
    ...
    combined = tf.keras.layers.Concatenate()([sasa_features, phi_features, psi_features, ss_features])
